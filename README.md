...
当前版本为 26.0830.03。版本号采用 `YY.MMDD.NN` 格式，依次表示两位年份、月日和当天第几次修改：
...
- 离线激活码使用 Ed25519 签名并绑定加密设备码和到期时间；`0` 天表示永久授权。许可证在 Application Support 和应用偏好设置中双份保存、可相互恢复，首次激活后更新无需再次输入。
- 防回拨状态同时保存在 Application Support 和应用偏好设置，并结合本次进程的单调运行时间检测系统时钟向后跳变；不访问登录钥匙串，更新后不会弹出钥匙串密码请求。
...
1. 下载或构建安装包后，打开 `dist/MacWindowButtons-26.0830.03.dmg`。
...
- 离线许可证可以阻止普通复制和篡改，但无法从理论上阻止攻击者修改应用二进制或同时删除两份本地授权状态；更高强度的授权需要联网校验服务。
