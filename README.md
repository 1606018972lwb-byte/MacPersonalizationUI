...
本地测试版本 1.1.1 已完成可操作的右侧窗口控制条、可视化控制中心和原创应用图标：
...
- 在焦点窗口顶部外侧、右边缘对齐的位置常驻显示不抢键盘焦点的半透明 `NSPanel`。
...
1. 构建本地测试包后，打开 `dist/MacWindowButtons-1.1.1.dmg`。
...
7. 返回微信并点击微信窗口，三个控制按钮会常驻显示在窗口上方右侧；使用工具按钮最大化后，顶部会保留一行用于显示控件。
