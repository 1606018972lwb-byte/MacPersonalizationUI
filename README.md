...
版本 1.1 已完成可操作的右侧窗口控制条、可视化控制中心和原创应用图标：
...
- 在焦点窗口标题栏下方的右侧边缘显示不抢键盘焦点的半透明 `NSPanel`。
- 默认自动隐藏控制条，只保留 7pt 蓝色提示条；提示条点击穿透，不会遮挡或拦截原应用操作。
- 鼠标移入蓝色提示条后展开三个按钮，离开约 0.9 秒自动收起。
- 主界面可以关闭自动隐藏；即使常显，控制条也不会再覆盖原应用标题栏按钮。
...
7. 返回微信并点击微信窗口，将鼠标移到标题栏下方的右侧蓝色提示条，三个控制按钮会向左展开。
