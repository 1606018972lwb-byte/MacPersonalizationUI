...
测试版本 1.4.4 基于正式版本 1.3，重做“与窗口一体”控制条并保留 Finder 文件快捷操作：
...
- 一体样式改用窗口背景材质，避免原标题栏材质受桌面颜色影响而与目标窗口产生明显色差。
- 一体控制条保留完整可见高度，并在底部增加 10pt 衬底；衬底排在目标窗口下一层，由原窗口自然覆盖并填满两个顶角缺口。
- 一体模式不再压住目标窗口内容或原生红黄绿按钮；无法确认跨进程窗口层级时自动取消重叠，优先保证不遮挡。
- 切换外观后立即生效并保存；最大化窗口始终预留完整的一行控制条高度。
...
1. 下载或构建安装包后，打开 `dist/MacWindowButtons-1.4.4.dmg`。
...
8. 如需调整图标大小，在设置窗口的“常规”页展开“按钮大小”，选择“小 / 标准 / 大”。
...
13. 在“其他”页选择“与窗口一体”，控制条会使用窗口背景色并把补角衬底放到目标窗口后方。
