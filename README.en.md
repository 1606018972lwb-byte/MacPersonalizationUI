...
The current version is 26.0830.03. Versions use `YY.MMDD.NN`: two-digit year, month and day, and the modification sequence for that day.
...
- Clock-rollback detection backed by Application Support and application preferences, plus monotonic uptime checks while the process is running. Login Keychain is no longer accessed, avoiding password prompts after local-signed updates.
...
- Finder now provides a “Copy Path” command for files, folders, and multiple selections; multiple paths are copied to the system pasteboard one per line.
- The Context Menu settings page can independently show or hide “Copy Path” and “Desktop Shortcut”; both are enabled by default.
